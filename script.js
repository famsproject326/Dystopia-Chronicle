...
window.addEventListener("touchstart", (e) => {
    ...
    if (gameMode === "HACKING") {
        // 戻るボタンの判定 (画面上部)
        if (t.clientY < 60) { gameMode = "EXPLORE"; return; }
...
function loop() {
    ...
    if (gameMode === "EXPLORE") {
        ...
        ctx.fillStyle = "#fff"; ctx.fillText("ROOM: " + roomIndex, window.innerWidth/2, 50);
        ...
        ctx.fillStyle = "#f00"; ctx.fillText("[BACK TO ROOM]", window.innerWidth/2, 40); // 戻るボタン表示
